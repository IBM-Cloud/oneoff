...
var run = function (queued, finished) {
  ...
  var wait_callback = function (task) {
    ...
    var wait_till_finished = function () {
      var wait_timer = if_not_done(function () {
        ...
        task._timeout = setTimeout(timeout_handler, TIMEOUT)
      })
      return wait_timer
    }

    return IS_INSTANCE_ZERO ? run_and_check_task() : wait_till_finished() 
  }
  ...
  fs.readdir(task_path, function (err, files) {
    ...
    var files = files.filter(function (file) {
      ...
      return require(path.join(task_path, file))
    })
